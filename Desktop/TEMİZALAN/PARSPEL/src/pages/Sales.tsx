  ...
  const handleReturn = (id: string) => {
    showConfirm('İade / İptal', 'Bu satışı iade etmek istiyor musunuz? Stoklar geri yüklenecek.', () => {
      ...
      save(prev => {
        ...
        if (cariId) {
          // Tüm satış tutarını cari'den düş (hem nakit tahsilat hem cari borç dahil)
          cari = cari.map(c => c.id === cariId
            ? { ...c, balance: (c.balance || 0) - (sale.total - tahsilEdilen), lastTransaction: nowIso, updatedAt: nowIso }
            : c
          );
        }
