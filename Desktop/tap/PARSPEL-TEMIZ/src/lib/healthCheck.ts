...
// ── Firebase ping URL (connConfig'den dinamik oku) ───────────────────────────
function getFirebasePingUrl(): string {
  const cfg = loadConnConfig();
  if (!cfg.firebase.enabled || !cfg.firebase.projectId || !cfg.firebase.apiKey) return '';
  return `https://firestore.googleapis.com/v1/projects/${cfg.firebase.projectId}/databases/(default)/documents/config/health?key=${cfg.firebase.apiKey}`;
}
...
async function checkFirebase(): Promise<HealthMetric> {
  ...
  const pingUrl = getFirebasePingUrl();
