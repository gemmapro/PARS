...
interface Emanet { id: string; partnerId: string; amount: number; note?: string; description?: string; type?: string; createdAt: string; }
    ...
    showConfirm('Ortağı Sil', 'Emin misiniz?', () => {
      ...
      save(prev => {
        ...
        const updatedEmanetler = (prev.ortakEmanetler || []).map((e) => {
          if (e.partnerId === id && !(e as { deleted?: boolean }).deleted) { emanetCount++; return { ...e, deleted: true, updatedAt: nowIso }; }
          return e;
        }) as import('@/types').OrtakEmanet[];
        return {
          ...prev,
